...
static void setuseragent(void)
{
	...
	uname(&un);
	len = snprintf(agent, 100, "bottle/%s (%s %s) libalpm/%s",
...
int main(int argc, char *argv[])
{
	...
	pm_printf(ALPM_LOG_DEBUG, "bottle v%s - libalpm v%s\n", PACKAGE_VERSION, alpm_version());
	...
	switch(config->op) {
		case PM_OP_DATABASE:
			ret = bottle_database(pm_targets);
			break;
		case PM_OP_REMOVE:
			ret = bottle_remove(pm_targets);
			break;
		case PM_OP_UPGRADE:
			ret = bottle_upgrade(pm_targets);
			break;
		case PM_OP_QUERY:
			ret = bottle_query(pm_targets);
			break;
		case PM_OP_SYNC:
			ret = bottle_sync(pm_targets);
			break;
		case PM_OP_DEPTEST:
			ret = bottle_deptest(pm_targets);
			break;
		case PM_OP_FILES:
			ret = bottle_files(pm_targets);
			break;
		default:
