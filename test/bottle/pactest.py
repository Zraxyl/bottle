#! /usr/bin/python3
#
#  pactest : run automated testing on the bottle binary
#
...
def create_parser():
    ...
    description = "Runs automated tests on the bottle binary. Tests are " \
    ...
    parser.add_option("-d", "--debug", type = "int",
                      ...
                      help = "set debug level for bottle")
    parser.add_option("-p", "--bottle", type = "string",
                      dest = "bin", default = None,
                      help = "specify location of the bottle binary")
...
if __name__ == "__main__":
    ...
    env.bottle["debug"] = opts.debug
    env.bottle["bin"] = opts.bin
    env.bottle["bindir"] = opts.bindir
    env.bottle["nolog"] = opts.nolog
    env.bottle["gdb"] = opts.gdb
    env.bottle["valgrind"] = opts.valgrind
    env.bottle["manual-confirm"] = opts.manualconfirm
    env.bottle["scriptlet-shell"] = opts.scriptletshell
    env.bottle["ldconfig"] = opts.ldconfig
    ...
    if not opts.keeproot:
        ...
        tap.diag("bottle testing root saved: %s" % root_path)
