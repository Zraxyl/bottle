...
class pmtest(object):
    ...
    def generate(self, bottle):
        ...
        ldconfig = os.path.basename(bottle["ldconfig"])
        ldconfigdir = os.path.join(self.root, os.path.dirname(bottle["ldconfig"][1:]))
        shell = bottle["scriptlet-shell"][1:]
    ...
    def add_hook(self, name, content):
        if not name.endswith(".hook"):
            name = name + ".hook"
        path = os.path.join("etc/bottle.d/hooks/", name)
    ...
    def run(self, bottle):
        if os.path.isfile(util.PM_LOCK):
            tap.bail("\tERROR: another bottle session is on-going -- skipping")
            return

        tap.diag("==> Running test")
        vprint("\tbottle %s" % self.args)
        ...
        prog = bottle["bin"]
        if not prog:
            prog = util.which(self.cmd[0], bottle["bindir"])
        ...
        if bottle["manual-confirm"]:
            cmd.append("--confirm")
        if bottle["debug"]:
            cmd.append("--debug=%s" % bottle["debug"])
        cmd.extend(shlex.split(self.args))

        if not (bottle["gdb"] or bottle["nolog"]):
        ...
        # Change to the tmp dir before running bottle, so that local package
        ...
        if os.path.isfile(os.path.join(self.root, util.TMPDIR, "core")):
            tap.diag("\tERROR: bottle dumped a core file")
