...
Synopsis
--------
{sysconfdir}/makepkg.conf, $XDG_CONFIG_HOME/bottle/makepkg.conf, ~/.makepkg.conf
...
$XDG_CONFIG_HOME/bottle/makepkg.conf or ~/.makepkg.conf, with the former
taking priority.
...
**PACMAN_AUTH=()**::
	Specify a command prefix for running bottle as root. If unset, makepkg will
...
See Also
--------
linkman:makepkg[8], linkman:bottle[8], linkman:PKGBUILD[5]
