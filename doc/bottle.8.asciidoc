...
Name
----
bottle - package manager utility


Synopsis
--------
'bottle' <operation> [options] [targets]


Description
-----------
Bottle is a package management utility that tracks installed packages on a Linux
...
automatically upgrade packages. Bottle packages are a zipped tar format.

Since version 3.0.0, bottle has been the front-end to linkman:libalpm[3], the
``Evolinx Package Management'' library. This library allows alternative
front-ends to be written (for instance, a GUI front-end).

Invoking bottle involves specifying an operation with any potential options and
...
*-D, \--database*::
	...
	certain attributes of the installed packages in bottle's database. It
...
*-S, \--sync*::
	...
	example, `bottle -S qt` will download and install qt and all the
	...
	package to install: `bottle -S testing/qt`. You can also specify version
	requirements: `bottle -S "bash>=3.2"`. Quotes are needed, otherwise the
	shell interprets ">" as redirection to a file.
+
In addition to packages, groups can be specified as well. For example, if
gnome is a defined package group, then `bottle -S gnome` will provide a
...
+
Packages that provide other packages are also handled. For example, `bottle -S
...
+
You can also use `bottle -Su` to upgrade all packages that are out-of-date. See
<<SO,Sync Options>> below. When upgrading, bottle performs version comparison
...
*-T, \--deptest*::
	...
	This operation accepts no other options. Example usage: `bottle -T qt
	"bash>=3.2"`.
...
*-U, \--upgrade*::
	...
	on how bottle takes care of configuration files.
...
*-b, \--dbpath* <path>::
	...
	+{localstatedir}/lib/bottle+). This should not be used unless you know what
	you are doing.
...
*-r, \--root* <path>::
	...
	*NOTE*: If database path or log file are not specified on either the
	command line or in linkman:bottle.conf[5], their default location will
...
*\--cachedir* <dir>::
	...
	+{localstatedir}/cache/bottle/pkg+). Multiple cache directories can be
	specified, and they are tried in the order they are passed to bottle.
...
*\--gpgdir* <dir>::
	...
	(the default is +{sysconfdir}/bottle.d/gnupg+). This directory should contain
...
*\--hookdir* <dir>::
	...
	+{sysconfdir}/bottle.d/hooks+). Multiple hook directories can be specified
...
*\--noconfirm*::
	...
	this unless you want to run bottle from a script.
...
*-d, \--nodeps*::
	...
	bottle will always check a package's dependency fields to ensure that all
...
*\--noprogressbar*::
	...
	for scripts that call bottle and capture the output.
...
*\--ignore* <package>::
	Directs bottle to ignore upgrades of package even if there is one
	available. Multiple packages can be specified by separating them
	with a comma.

*\--ignoregroup* <group>::
	Directs bottle to ignore upgrades of all packages in 'group', even if
...
*-q, \--quiet*::
	...
	bottle's output is processed in a script. Search will only show package
...
*-n, \--nosave*::
	Instructs bottle to ignore file backup designations. Normally, when a
...
*-c, \--clean*::
	...
	currently unused sync databases to free up disk space. When bottle
	...
	linkman:bottle.conf[5]. Use one '\--clean' switch to only remove
...
*-u, \--sysupgrade*::
	...
	necessary.
+
Pass this option twice to enable package downgrades; in this case, bottle will
...
*-y, \--refresh*::
	...
	server(s) defined in linkman:bottle.conf[5]. This should typically be used
...
*-q, \--quiet*::
	...
	bottle's output is processed in a script, however, you may want to use
...
----------------------------
Bottle uses the same logic as 'rpm' to determine action against files that are
...
bottle -U /home/user/ceofhack-0.6-1-x86_64.pkg.tar.gz::
...
Configuration
-------------
See linkman:bottle.conf[5] for more details on configuring bottle using the
'bottle.conf' file.
