...
customizations, generate meta-info, and package the whole thing up for bottle
to use.
...
*-s, \--syncdeps*::
	Install missing dependencies using bottle. When build-time or run-time
	dependencies are not found, bottle will try to resolve them. If
...
*\--noconfirm*::
	(Passed to bottle) Prevent bottle from waiting for user input before
	proceeding with operations.

*\--needed*::
	(Passed to bottle) Tell bottle not to reinstall a target if it is already
	up-to-date. (used with '-i' / '\--install').

*\--asdeps*::
	(Passed to bottle) Install packages as non-explicitly installed (used
	with '-i' / '\--install').

*\--noprogressbar*::
	(Passed to bottle) Prevent bottle from displaying a progress bar;
...
**PACMAN**::
	...
	install and remove packages. Bottle's '-Qq', '-Rns', '-S', '-T', and '-U'
	operations must be supported by this command. If the variable is not
	set or empty, makepkg will fall back to `bottle'.
...
See Also
--------
linkman:makepkg.conf[5], linkman:PKGBUILD[5], linkman:bottle[8]
