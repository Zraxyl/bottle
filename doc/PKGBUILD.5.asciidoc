...
*epoch*::
	...
	version comparison logic. See linkman:bottle[8] for more information on
	version comparisons.
...
*backup (array)*::
	...
	`"Handling Config Files"` in linkman:bottle[8] for more information.
...
*optdepends (array)*::
	...
	and are not utilized by bottle during dependency resolution. Packages in
...
+
Sysupgrade is currently the only bottle operation that utilizes this field.
...
--------------------------------
Bottle has the ability to store and execute a package-specific script when it
...
See Also
--------
linkman:makepkg[8], linkman:bottle[8], linkman:makepkg.conf[5]
